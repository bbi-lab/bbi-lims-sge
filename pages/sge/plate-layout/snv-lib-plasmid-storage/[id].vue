...
const plateLayout = usePlateLayout<{ snvLibPlasmid: SnvLibPlasmid | null }>()
...
onMounted(async() => {
    ...
    plateLayout.wellContentsDisplayConfig.value = {
        colorBy: [(wellContent: any) => _.get(wellContent, 'snvLibPlasmid.id')],
        selectionTableRecordIdPaths: ['snvLibPlasmid.id'],
        tooltip: (well: any) => {
            const wellCoordinate = `${wellCoordinateToChar(well.y)}${well.x}`
            const productName = _.get(well, ['wellContents', 0, 'wellable', 'snvLibPlasmid', 'name'])
            return productName ? `${wellCoordinate}:<br>${productName} (SNVlib Plasmid)` : wellCoordinate
        },
    }
    loadPlate()
})
