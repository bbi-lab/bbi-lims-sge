...
import type { SnvLibLinProduct } from '~/server/db/schema/sge/oligos'
import { getWellTextColor, wellCoordinateToChar } from '~/lib/plate-diagram'

const { breakpoints } = useLayout()
const route = useRoute()
const plateLayout = usePlateLayout<{ snvLibLinProduct: SnvLibLinProduct | null }>()
...
onMounted(async() => {
    ...
    plateLayout.wellContentsDisplayConfig.value = {
        colorBy: [(wellContent: any) => _.get(wellContent, 'snvLibLinProduct.id')],
        selectionTableRecordIdPaths: ['snvLibLinProduct.id'],
        tooltip: (well: any) => {
            const wellCoordinate = `${wellCoordinateToChar(well.y)}${well.x}`
            const productName = _.get(well, ['wellContents', 0, 'wellable', 'snvLibLinProduct', 'name'])
            return productName ? `${wellCoordinate}:<br>${productName} (SNVlib LIN product)` : wellCoordinate
        },
    }
    loadPlate()
})
...
const columnDefs = {
    ...
    dpn1DigestOn: { header: 'DpnI digest on' },
    dpn1DigestBy: { header: 'DpnI digest by', path: 'dpn1DigestBy.name' },
    gelExtractedBy: { path: 'gelExtractedBy.name' },
