...
onMounted(async() => {
    ...
    plateLayout.wellContentsDisplayConfig.value = {
        colorBy: [(wellContent: any) => _.get(wellContent, 'haPcrProduct.id')],
        selectionTableRecordIdPaths: ['haPcrProduct.id'],
        tooltip: (well: any) => {
            const wellCoordinate = `${wellCoordinateToChar(well.y)}${well.x}`
            const productName = _.get(well, ['wellContents', 0, 'wellable', 'haPcrProduct', 'name'])
            return productName ? `${wellCoordinate}:<br>${productName} (HA PCR product)` : wellCoordinate
        },
    }
    loadPlate()
})
...
const columnDefs = {
    ...
    wtHap1DnaConcentration: { display: false },
    temperatureChosen: { display: false },
    performedBy: { path: 'performedBy.name' },
    targets: {
        format: (data: any) => {
            return _.map(data.haCloningExperiment?.haCloningExperimentTargets || [], 'target.name').join(', ')
        },
