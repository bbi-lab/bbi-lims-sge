...
const plateLayout = usePlateLayout<{ rnaPreseq2Primer: RnaPreseq2Primer | null }>()
...
onMounted(async() => {
    ...
    plateLayout.wellContentsDisplayConfig.value = {
        colorBy: ['rnaPreseq2Primer.rnaPreseq2PrimerTargets'],
        selectionTableRecordIdPaths: ['rnaPreseq2Primer.id'],
        tooltip: (well: any) => {
            const wellCoordinate = `${wellCoordinateToChar(well.y)}${well.x}`
            const primerName = _.get(well, ['wellContents', 0, 'wellable', 'rnaPreseq2Primer', 'name'])
            return primerName ? `${wellCoordinate}:<br>${primerName} (PCR2)` : wellCoordinate
        },
        symbol: (well: any) => {
            const primerDirection = _.get(well, ['wellContents', 0, 'wellable', 'rnaPreseq2Primer', 'sequenceType'])
    ...
    plateLayout.setExportPlateLayoutConfig({
        columns: [
            ...
            { header: 'Primer', data: (well: any) => {
                return _.get(well, ['wellContents', 0, 'wellable', 'rnaPreseq2Primer', 'name']) || ''
            }},
            { header: 'Project', data: (well: any) => {
                return _.uniq(_.map(_.get(well, ['wellContents', 0, 'wellable', 'rnaPreseq2Primer', 'rnaPreseq2PrimerTargets']) || [], 'target.project.name')).join(', ')
            }},
            { header: 'Target', data: (well: any) => _.map(_.get(well, ['wellContents', 0, 'wellable', 'rnaPreseq2Primer', 'rnaPreseq2PrimerTargets']) || [], 'target.name').join(', ') },
        ],
        sortBy: (well: any) => {
            return `${wellCoordinateToChar(well.y)}${well.x}`
        }
