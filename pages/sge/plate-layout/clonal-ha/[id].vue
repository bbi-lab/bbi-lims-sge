...
import type { ClonalHa } from '~/server/db/schema/sge/oligos'
import { getWellTextColor, wellCoordinateToChar } from '~/lib/plate-diagram'

const { breakpoints } = useLayout()
const route = useRoute()
const plateLayout = usePlateLayout<{ clonalHa: ClonalHa | null }>()
...
onMounted(async() => {
    ...
    plateLayout.wellContentsDisplayConfig.value = {
        colorBy: [(wellContent: any) => _.get(wellContent, 'clonalHa.name')],
        selectionTableRecordIdPaths: ['clonalHa.id'],
        tooltip: (well: any) => {
            const wellCoordinate = `${wellCoordinateToChar(well.y)}${well.x}`
            const clonalHaName = _.get(well, ['wellContents', 0, 'wellable', 'clonalHa', 'name'])
            return clonalHaName ? `${wellCoordinate}:<br>${clonalHaName}` : wellCoordinate
        },
    }
    loadPlate()
})
