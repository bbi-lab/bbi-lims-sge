...
import type { HaPuc19PcrProduct } from '~/server/db/schema/sge/oligos'
import { getWellTextColor, wellCoordinateToChar } from '~/lib/plate-diagram'

const { breakpoints } = useLayout()
const route = useRoute()
const plateLayout = usePlateLayout<{ haPuc19PcrProduct: HaPuc19PcrProduct | null }>()
...
onMounted(async() => {
    ...
    plateLayout.wellContentsDisplayConfig.value = {
        colorBy: [(wellContent: any) => _.get(wellContent, 'haPuc19PcrProduct.id')],
        selectionTableRecordIdPaths: ['haPuc19PcrProduct.id'],
        tooltip: (well: any) => {
            const wellCoordinate = `${wellCoordinateToChar(well.y)}${well.x}`
            const productName = _.get(well, ['wellContents', 0, 'wellable', 'haPuc19PcrProduct', 'name'])
            return productName ? `${wellCoordinate}:<br>${productName} (HA pUC19 PCR product)` : wellCoordinate
        },
    }
    loadPlate()
})
...
const columnDefs = {
    ...
    targets: {
        format: (data: any) => {
            return _.map(data.haPcrProduct?.haCloningExperiment?.haCloningExperimentTargets || [], 'target.name').join(', ')
        },
