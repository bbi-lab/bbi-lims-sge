...
import type { HaPuc19GibsonProduct } from '~/server/db/schema/sge/oligos'
import { getWellTextColor, wellCoordinateToChar } from '~/lib/plate-diagram'

const { breakpoints } = useLayout()
const route = useRoute()
const plateLayout = usePlateLayout<{ haPuc19GibsonProduct: HaPuc19GibsonProduct | null }>()
...
onMounted(async() => {
    ...
    plateLayout.wellContentsDisplayConfig.value = {
        colorBy: [(wellContent: any) => _.get(wellContent, 'haPuc19GibsonProduct.id')],
        selectionTableRecordIdPaths: ['haPuc19GibsonProduct.id'],
        tooltip: (well: any) => {
            const wellCoordinate = `${wellCoordinateToChar(well.y)}${well.x}`
            const productName = _.get(well, ['wellContents', 0, 'wellable', 'haPuc19GibsonProduct', 'name'])
...
const columnDefs = {
    ...
    puc19VectorConcentration: { display: false },
    puc19VectorAmount: { display: false },
    // temperatureUsed: { display: false },
    preppedBy: { path: 'cleanedBy.name' },
    targets: {
        format: (data: any) => {
            return _.map(data.haPuc19PcrProduct?.haPcrProduct?.haCloningExperiment?.haCloningExperimentTargets || [], 'target.name').join(', ')
        },
