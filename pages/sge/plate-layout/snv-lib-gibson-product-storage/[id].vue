...
onMounted(async() => {
    ...
    plateLayout.wellContentsDisplayConfig.value = {
        colorBy: [(wellContent: any) => _.get(wellContent, 'snvLibGibsonProduct.id')],
        selectionTableRecordIdPaths: ['snvLibGibsonProduct.id'],
        tooltip: (well: any) => {
            const wellCoordinate = `${wellCoordinateToChar(well.y)}${well.x}`
            const productName = _.get(well, ['wellContents', 0, 'wellable', 'snvLibGibsonProduct', 'name'])
            return productName ? `${wellCoordinate}:<br>${productName} (SNVlib Gibson product)` : wellCoordinate
        },
    }
    loadPlate()
})
...
const columnDefs = {
    ...
    linProductVectorAmount: { display: false },
    gibsonBy: { path: 'gibsonBy.name' },
    cleanedOn: { display: false },
    cleanedBy: { display: false },
    transformedBy: { display: false },
    transformedOn: { display: false },
    preppedOn: { display: false },
    preppedBy: { display: false },
    benchlingLink: { display: false },
    plasmidsaurusChecked: { display: false },
    ngsChecked: { display: false },
    passedQc: { display: false },
    totalReactionVolume: { display: false },
    wellContents: {
        header: 'Wells',
        format: (x: any) => {
