...
const submitRnaPreseq1Primers = async (data: any[]) => {
    try {
        ...
        const response: { primers: { id: string }[]; insertedCount: number } = await $fetch(`${config.public.apiBase}/custom/primers/rna-preseq1-primers/pcr-primer-import`, {
            method: 'POST',
            body: filteredData,
        })
        crudTable.didAddRecords(response.primers)
        toast.add({ severity: 'success', summary: 'Success', detail: `Successfully imported ${response.insertedCount} RNA PreSeq 1 Primers.`, life: 5000 })
...
const importRnaPreseq1Primers = (event: any) => {
    try {
        const files = event.files
        const f = files[0]

        fileToSheet(f, submitRnaPreseq1Primers)
    } catch (error) {
        console.error('Error importing RNA PreSeq 1 Primers:', error)
    }
}
...
<template>
    ...
    <Dialog v-model:visible="importDialogVisible" modal :closable="false" :style="{ width: '35' }">
        ...
        <slot name="header">
            <span class="flex justify-center mt-3 font-bold">Import RNA PreSeq 1 Primers from CSV</span>
        </slot>
        ...
        <FileUpload
            ...
            :customUpload="true"
            :auto="true"
            @uploader="importRnaPreseq1Primers"
            chooseLabel="Upload"
            v-tooltip="{value: 'Upload RNA PreSeq 1 Primers', showDelay: 500}"
        >
            <template #chooseicon>
