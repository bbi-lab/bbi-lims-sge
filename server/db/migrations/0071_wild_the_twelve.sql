...
ALTER TABLE "plates" ADD COLUMN "sequencing_run_id" uuid;--> statement-breakpoint
ALTER TABLE "plates" ADD CONSTRAINT "plates_sequencing_run_id_sequencing_runs_id_fk" FOREIGN KEY ("sequencing_run_id") REFERENCES "public"."sequencing_runs"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "plates" ADD CONSTRAINT "sequencing_run_plate_type_check" CHECK (("plates"."sequencing_run_id" IS NULL AND "plates"."plate_type" != 'preseq-3') OR "plates"."plate_type" = 'preseq-3');--> statement-breakpoint
CREATE VIEW "public"."view_plates_with_well_counts" AS (with plate_types(plate_type_value, plate_type_label, plate_type_desc) AS (VALUES ('pellet-storage', 'Pellet storage', 'Pellet storage'), ('lin-storage', 'LIN primer storage', 'Linearization primer storage'), ('amp-storage', 'AMP primer storage', 'Amplification primer storage'), ('ha-storage', 'HA primer storage', 'Homology arm primer storage'), ('guide-rna-storage', 'Guide RNA storage', 'Guide RNA storage'), ('lin-pcr', 'LIN PCR', 'Linearization primer PCR'), ('amp-pcr', 'AMP PCR', 'Amplification primer PCR'), ('ha-pcr', 'HA PCR', 'Homology arm primer PCR'), ('preseq-1', 'PreSeq 1', 'PreSeq 1'), ('preseq-2', 'PreSeq 2', 'PreSeq 2'), ('preseq-3', 'PreSeq 3', 'PreSeq 3'), ('seq-index', 'Seq index', 'Sequencing index plate')) select
    ...
    (select name from sequencing_runs where "sequencing_runs"."id" = "plates"."sequencing_run_id") as sequencing_run_name
    from "plates"
