CREATE VIEW "public"."view_sequencing_run_well_contents" AS (SELECT
    ...
    array_agg("index_primers"."id" ORDER BY "index_primers"."id") FILTER (WHERE "index_primers"."id" IS NOT NULL) AS index_primer_ids,
    array_agg("index_primers"."index_sequence" ORDER BY "index_primers"."id") FILTER (WHERE "index_primers"."id" IS NOT NULL) AS index_primer_sequences,
